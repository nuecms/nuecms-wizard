...
export class Importer {
  ...
  _conn: mysql.Connection | null;
  _encoding: string;
  _imported: any[];
  _progressCB: (progress: any) => void;
  _dumpCompletedCB: (result: { total_files: number, file_no: number, file_path: string, error: any }) => void;
  _total_files: number;
  _current_file_no: number;
  ...
  _importSingleFile(fileObj) {
    return new Promise<void>((resolve, reject) => {
      ...
      readerStream.setEncoding(this._encoding as BufferEncoding);
   ...
   */
  _connect() {
    return new Promise<void | mysql.Connection>((resolve, reject) => {
      if (this._conn) {
        resolve(this._conn);
        return;
      }
   ...
   * @returns {Promise}
   */
  _statFile(filepath): Promise<fs.Stats> {
   ...
   * @returns {Promise}
   */
  _getSQLFilePaths(...paths): Promise<{ file: string, size: number }[]> {
      ...
      for (let i = 0; i < paths.length; i++) {
        ...
        await new Promise<void>(async next => {
          ...
          try {
            ...
            var stat: fs.Stats = await this._statFile(filepath);
            if (stat.isFile()) {
              ...
              var more_paths = await this._readDir(filepath) as string[];
              more_paths = more_paths.map(p => path.join(filepath, p));
              var sql_files: { file: string, size: number }[] = await this._getSQLFilePaths(...more_paths);
